...
void EntityRenderer::drawMirror(const Entity& entity,
                const Camera& camera,
                const Lights& lights,
                const Skybox& skybox,
                const RenderOptions& options)
{
    ...
    d_mirrorShader.loadUniform("transformMatrix", transform);
    d_mirrorShader.loadUniform("projectionMatrix", projection);
    d_mirrorShader.loadUniform("viewMatrix", view);

     // Load remaining entity to shader
	d_mirrorShader.loadUniform("shineDamper", entity.texture()->shineDamper());
	d_mirrorShader.loadUniform("reflectivity", entity.texture()->reflectivity());

    // Load lights to shader
    for (size_t i = 0; i != MAX_NUM_LIGHTS; ++i) {
		if (i < lights.size()) {
			d_mirrorShader.loadUniform(arrayName("lightPositions", i), lights[i].position);
			d_mirrorShader.loadUniform(arrayName("lightColours", i), lights[i].colour);
			d_mirrorShader.loadUniform(arrayName("lightAttenuations", i), lights[i].attenuation);
		}
		else {  // "Empty" lights to pad the array
			d_mirrorShader.loadUniform(arrayName("lightPositions", i), {0.0f, 0.0f, 0.0f});
			d_mirrorShader.loadUniform(arrayName("lightColours", i), {0.0f, 0.0f, 0.0f});
			d_mirrorShader.loadUniform(arrayName("lightAttenuations", i), {1.0f, 0.0f, 0.0f});
		}
	}
