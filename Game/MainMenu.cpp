    ...
    , d_renderer(accessor.window(), accessor.resourceManager())
{
    ...
    button->setUnclickCallback([&]() {
        if (!d_accessor.doesSceneExist("World")) {
            auto info = std::make_shared<BasicSceneInfo>(accessor);
            auto worldStack = d_accessor.addScene("World");
            worldStack->add<WorldLayer>(accessor, info);
            worldStack->add<UILayer>(accessor, info);
        }
        
        accessor.setActiveScene("World");
    });
}
